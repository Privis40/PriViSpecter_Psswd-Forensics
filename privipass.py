...
class PriViPDFReport(FPDF):
    ...
    def generate_report(self, results, batch=False, operator=None):
        self.add_page()
        self.set_text_color(0, 0, 0)
        if batch:
            self._write_batch_section(results, operator)
        else:
            self._write_single_section(results, operator)

        # Signature
        self.ln(8)
        self.set_font("Helvetica", "I", 9)
        self.set_text_color(100, 100, 100)
        self.cell(0, 6, f"Powered by {BRAND}  |  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
    ...
    def _write_single_section(self, r, operator=None):
        self.section_title("1. Executive Risk Rating")
        op_line = f"Conducted by: {(operator or {}).get('name', 'Operator')}"
        if operator and operator.get("org"):
            op_line += f"  |  {operator['org']}"
        self.set_font("Helvetica", "I", 9)
        self.set_text_color(100, 100, 100)
        self.cell(0, 6, op_line, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        self.set_text_color(0, 0, 0)
        self.ln(2)
        score = r.get("score", 1)
        score_map = {
    ...
    def _write_batch_section(self, results, operator=None):
...
class PriViSpecter:
    ...
    def audit(self, password, operator=None):
        self.matrix_effect()
        ...
        if choice == "1":
            ...
            pdf   = PriViPDFReport()
            fname = pdf.generate_report(result_data, batch=False, operator=operator)
    ...
    def batch_audit(self, filepath, operator=None):
        ...
        pdf   = PriViPDFReport()
        fname = pdf.generate_report(results, batch=True, operator=operator)
...
def get_operator_info() -> dict:
    """Prompt for operator name. Appears in PDF as Conducted by."""
    console.print(Panel(
...
def authorization_gate():
    ...
    console.print(Panel(gate_text, border_style="red",
                        title="[bold red]PriViSpecter | PriViSecurity[/bold red]"))
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.wordlist:
            app.batch_audit(args.wordlist, operator)
        else:
